...
HELP_MSG = "Run a shell in the build environment of a Moonforge project"

HELP_DESCRIPTION = """The shell command sets up a containerized environment and runs a shell inside it for the Moonforge project at the given PATH.
...
def shell_setup(
    ...
    log.info(f"Shell for project {project.name} at {project.path}")

    return container.setup_container(
        project,
        source,
        container.ContainerCommand.SHELL,
        interactive=True,
        writable=True,
        fragments=fragments,
...
def run(options):
    ...
    for frag in options.fragments:
        ...
        fragments.append(p)
    return shell_setup(project, source, fragments, options.runtime_args)
